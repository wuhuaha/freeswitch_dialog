...
link_node_t new_node_entry( 
    void * value, 
    link_node_t prev, 
    link_node_t next ) 
{
    link_node_t node = ( link_node_t ) 
        malloc( sizeof( link_node_entry ) );
    if ( node == NULL ) {
...
void free_link_list( link_t link ) 
{
...
//clear all nodes and value in the link list and free all node and value
link_t link_list_clear_free( 
    link_t link ) 
{
    link_node_t node, next;
    //free all the middle nodes
...
//FRISO_API uint_t link_list_size( link_t link ) {
//    return link->size;
//}

//check if the link list is empty
//FRISO_API int link_list_empty( link_t link ) {
//    return ( link->size == 0 );
//}
 ...
 * static
 */
link_node_t get_node( 
    link_t link, uint_t idx ) 
{
    link_node_t p = NULL;
    register uint_t t;
...
void link_list_add( 
    link_t link, void * value ) 
{
    insert_before( link, link->tail, value );
}

//add a new node before the given index.
 void link_list_insert_before( 
    link_t link, uint_t idx, void * value  ) 
{
